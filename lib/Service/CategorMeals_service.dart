...
Future<List<CategoryMealsModel>> categoryMealsFetch(int id) async {
  // var token=await getTokenValue();
  // print(token);
  http.Response response = await http.get(
      'https://tastewind.com/api/menus/$id',
      headers:{
  ...
  if (response.statusCode == 200) {
    ...
    var responseData=responseJson['menus'];
    List<CategoryMealsModel> categoryList=[];
    for(var item in responseData){
      categoryList.add(CategoryMealsModel.fromJson(item));
    }
    return categoryList;
